...
//		}
		...
		if( change > 0 ) {
			...
			//메소드를 한 개 선언한 후 메소드를 호출하도록 변경한다.
			change = calculateChange( change, 5000 );
	...
	static int calculateChange(int change, int unit) {
		int count = change / unit; //단위별 갯수
		change = change % unit; //단위별 갯수를 제하고 난 나머지
		if( count > 0 ) { 
